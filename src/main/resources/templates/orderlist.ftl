    ...
    <#list infos as info>
        <div class="card border-dark my-3">
            ...
            <p>Им'я замовника: ${info.name}</br>
                ...
                <br>
                Номер телефону кур'єра: ${info.getUser().getPhonenumber()}
            </p>
            <form action="/orderlist" method="get">
                <button type="submit" name="order" value="${info.getId()}" class="btn btn-outline-dark btn-block">${info.getStatus().getStatus_name()}</button>
            </form>

        </div>
        </div>
    <#else>
        Користувач не має взятих замовлень.
    </#list>
